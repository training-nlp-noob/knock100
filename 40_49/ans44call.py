...
for i,sentence in enumerate(essay) :
    ...
    for chunks in sentence:
        if chunks.dst != -1:
            ...
            print( moto + "\t" + saki)
            if len(moto) >=1 and len(saki) >= 1:
                G.add_edge(moto,saki)
    G.layout()
    G.draw("./Q44_out/ans%s.png"%i)


"""
if len(moto) >=1 and len(saki) >= 1:
は 18文目の最初の空白が かかり元なし かかり先ありとなっているため
エラー回避
800ファイルぐらいまでやったところでCtrl+C でアボートした
"""
